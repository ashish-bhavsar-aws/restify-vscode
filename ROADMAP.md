...
| F38 | **Documentation generation** | ⚪ P3 | ✅ Publish a human-readable HTML/Markdown docs page from a collection. *(done — `src/core/docsGenerator.ts` pure module generating Markdown + HTML from collection endpoints, params, headers, body, groups, variables, scripts; `restify.generateDocs` command with collection picker + opens in editor; 14 unit tests in `test/unit/docsGenerator.test.ts`)* |
...
| F55 | **Rich diff of saved request** | ⚪ P3 | ✅ Show unsaved-change indicator vs last saved (there's a dirty dot only). *(done — `TabState.savedSnapshot` stores request at save time; `computeDirtyFields` compares current vs snapshot (URL, method, body, headers, params, auth); TopBar dirty dot tooltip shows changed field names)* |
| F56 | **Undo/redo in body editor** | ⚪ P3 | ✅ CodeMirror history is present; ensure it's surfaced with shortcuts. *(done — textarea-based editor already supports Ctrl+Z/Ctrl+Shift+Z natively; added visible Undo/Redo toolbar buttons in `CodeEditor.tsx` for discoverability)* |
| F57 | **History search & pins** | 🟡 P2 | Persist search query, pin favorites, fuzzy search across history. |
| F58 | **Screenshots/theme polish** | 🟡 P2 | ✅ Icon themes, method color on request rows, empty-state CTAs. *(done — main-panel tabs render a colored HTTP method chip (per-method colors via the existing `--tag-*` CSS variables) with the method stripped from the in-panel label (window title keeps it); the empty states now use themed FA icons + accent CTA buttons — **New Request** on History (posts `newRequest` → `restify.newRequest`, handled in `SidebarProvider`) and **New Collection** on Collections (opens the new-collection modal); E2E already covered by `palette-commands.spec.ts`)* |
...
- [x] F36 OpenAPI explorer, [x] F37 mock server, [x] F38 docs generation.
