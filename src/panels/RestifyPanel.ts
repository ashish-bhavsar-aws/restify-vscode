...
export class RestifyPanel {
  ...
  private _tabControllers = new Map<string, AbortController>();
  ...
  private async _handleMessage(msg: any): Promise<void> {
    switch (msg.command) {
      ...
      case "executeRequest":
        ...
        await this._executeRequest(msg.request, msg.savedRequest, msg.tabId);
        break;
      case "getOAuthToken":
        await this._handleGetOAuthToken(msg.config, msg.tabId);
        break;
      case "cancelRequest":
        if (msg.tabId) this._tabControllers.get(msg.tabId)?.abort();
        else this._cancelActiveRequest();
        break;
  ...
  private async _executeRequest(
    ...
    this.panel.webview.postMessage({ command: "requestStart", tabId });
    ...
    try {
      ...
      if (this._tabControllers.get(tabId) === controller) {
        this._tabControllers.delete(tabId);
      }
    }
  }
